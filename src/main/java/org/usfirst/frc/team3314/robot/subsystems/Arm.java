...
public class Arm implements Subsystem {
	...
	public Arm() {
		...
		if(telescopeTalon.getSensorCollection().getPulseWidthPosition() >= Math.abs(Constants.kTelescopeEncoderOffset - 200)) {
			...
			telescopeTalon.setSelectedSensorPosition(((telescopeTalon.getSensorCollection().getPulseWidthPosition()  +  (4096 -Constants.kTelescopeEncoderOffset))), 0, Constants.kCANTimeout);
		}
		...
		armTalon.setSensorPhase(false); //Comp: true practice: false
	...
	public void startUp() {
		...
		armTalon.set(ControlMode.MotionMagic, getArmAngleTicks());
		telescopeTalon.set(ControlMode.MotionMagic, getTelescopePositionTicks());
	...
	public void update() {
		...
		case TO_SCALE_HIGH:
			...
			break;
		case TO_SCALE_AUTO:
			targetArmAngle = Constants.kScaleAutoArmAngle;
		...
		case LOWER_TO_BAR:
			...
			if(inPosition()) {
				currentState = ArmState.DETACH_HOOKS;
			}
			break;
		case DETACH_HOOKS:
			targetTelescopePosition = Constants.kTelescopeMinPosition;
			if(inPosition()) {
				currentState = ArmState.TO_SCALE_LOW;
			}
			break;
		case FLIP_CUBE:
			targetArmAngle = Constants.kPickUpAngle;
